...
Consider reordered or duplicate work, retries, concurrency, partial writes, stale state, mixed versions, migration order, alternate authorization paths, dependency outages, timeouts, restart, replay, rollback, and recovery only when relevant. Report a defect only when a permitted input or reachable state follows a complete path to an observable failure. Do not invent a deployment condition, external behavior, or contract to make a concern concrete. Do not turn generic hardening ideas into defects.
